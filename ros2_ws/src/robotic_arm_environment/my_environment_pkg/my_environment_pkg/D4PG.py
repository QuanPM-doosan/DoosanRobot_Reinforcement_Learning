...
class D4PGAgent:
    ...
    def __init__(
        ...
        tau=1e-3,
        n_steps=1,
        learn_every=100,  # số bước mới học một lần (giữ mặc định 100 cho Pendulum)
    ):
    ...
    def get_action(self, state, use_noise=True):
        """
        state: numpy (obs_dim,)
        use_noise: True trong TRAIN, False trong DEMO nếu muốn deterministic.
        """
        state_tensor = torch.from_numpy(state).float().unsqueeze(0)  # numpy to a tensor with shape [1,obs_dim]
    ...
    def step_training(self, state, action, reward, next_state, done, batch_size, per_memory_status):
        """
        Thêm 1 sample vào memory và, mỗi `learn_every` bước, gọi learn_step().
        """
    ...
    def learn_step(self, batch_size, per_memory_status):
        ...
        # update the target networks using tau "soft updates"
        for target_param, param in zip(self.actor_target.parameters(), self.actor.parameters()):
    ...
    # ---------- SAVE / LOAD cho ROS2 env ----------

    def save(self, prefix, episode=None):
        """
        Lưu 1 file duy nhất: prefix + _epXXXX.pt nếu có episode,
        ngược lại chỉ prefix + '.pt'.
        """
        if episode is None:
            suffix = ""
        else:
            suffix = f"_ep{int(episode):04d}"
        path = prefix + suffix + ".pt"
        data = {
        ...
        print(f"[D4PG] Saved checkpoint to: {path}")

    def load_from_file(self, path):
        """
        Load từ 1 file duy nhất được lưu bởi save().
        """
    ...
    for episode in range(1, EPISODES + 1):
        ...
        while not done:
            ...
            for n in range(rollout_steps):
                action = agent.get_action(state, use_noise=True)
