...
class MyRLEnvironmentNode(Node):
    ...
    def __init__ (self):
        ...
        # Đăng ký callback mới: chỉ dùng JointState, không dùng my_sphere
        ...
        # Khởi tạo biến để tránh AttributeError nếu đọc sớm
    ...
    # --------------------------Callback mới: chỉ dùng JointState--------------------------#
    def initial_callback(self, joint_state_msg):

        # Mapping theo thứ tự dữ liệu gốc
        ...
        if len(joint_state_msg.velocity) >= 6:
            self.joint_1_vel = joint_state_msg.velocity[2]
            self.joint_2_vel = joint_state_msg.velocity[0]
            self.joint_3_vel = joint_state_msg.velocity[1]
            self.joint_4_vel = joint_state_msg.velocity[3]
        ...
        # Cập nhật vị trí end-effector từ TF
        ee_pose = self.get_end_effector_transformation()
        if ee_pose is not None:
            self.robot_x, self.robot_y, self.robot_z = ee_pose
    ...
    def get_end_effector_transformation(self):
        ...
        try:
            ...
            # Rotation (không dùng hiện tại nhưng giữ lại)
    ...
    def reset_environment_request(self):
        ...
        # -------------------- reset sphere position (ẨN HOÀN TOÀN)------------------#
    ...
    def generate_action_funct(self):
        ...
        angle_j_1 = random.uniform( -3.14159, 3.14159)
        angle_j_2 = random.uniform( -0.57595, 0.57595)
        angle_j_3 = random.uniform( -2.51327, 2.51327)
        angle_j_4 = random.uniform( -3.14159, 3.14159)
        angle_j_5 = random.uniform( -3.14159, 3.14159)
        angle_j_6 = random.uniform( -3.14159, 3.14159)

        return [angle_j_1, angle_j_2, angle_j_3, angle_j_4, angle_j_5, angle_j_6]


    # --------------------------Reward cũ dùng sphere (ẨN)--------------------------#
    ...
    # --------------------------Reward mới: không dùng my_sphere--------------------------#
    def calculate_reward_funct(self):
        try:
            robot_end_position = np.array((self.robot_x, self.robot_y, self.robot_z))
        except:
            self.get_logger().info('could not calculate reward yet, trying again...')
            return
        else:
            # Tạm thời: không có target cố định -> reward = 0, done=False
            done = False
            reward_d = 0.0
            return reward_d, done


    # --------------------------State-space cũ dùng sphere (ẨN)--------------------------#
    ...
    # --------------------------State-space mới: chỉ robot & joints--------------------------#
    def state_space_funct(self):
        try:
            state = [
                self.robot_x, self.robot_y, self.robot_z,
            ...
            self.get_logger().info(
                f'-------node not ready yet, Still getting values------------------ ({e})'
            )
            return
        else:
            return state
