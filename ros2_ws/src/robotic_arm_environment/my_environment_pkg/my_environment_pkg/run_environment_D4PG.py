...
from .main_rl_environment_D4PG import (
    MyRLEnvironmentNode, scale_action_unit_to_joint,
    J_MIN, J_MAX
)
from .D4PG import D4PGAgent
...
NUM_EPISODES_DEMO  = 30            
EPISODE_HORIZON    = 3

# =============== RL PARAMS ==================
BATCH_SIZE      = 32
PER_MEMORY_FLAG = False   # hiện tại chưa dùng PER, giữ False

# ================== LOG =====================
VERBOSE_RL      = True         # in log cho mỗi bước
LOG_EVERY_STEP  = 1

# ====== ĐƯỜNG DẪN CHECKPOINT & LOG =========
CKPT_PREFIX = os.path.expanduser(
    "~/QuanPM_robotic_arm_ws/ros2_ws/rl_checkpoints/d4pg_doosan"
)
...
RUN_TAG_TRAIN = "d4pg_train"
RUN_TAG_DEMO  = "d4pg_demo"
...
def _ee_from_state(s_vec):
    # state: [ee_x,ee_y,ee_z, j1..j6, sphere_x,y,z, obs_x,y,z]
    return np.array(s_vec[0:3], dtype=np.float64)


# ====== Fake env cho D4PG (shape) ======
class _SimpleSpace:
...
def set_mode_by_distance(dist_out_prev: float):
    """Đổi config chuyển động theo khoảng cách EE–AABB."""
    global _cur_cfg
    if not np.isfinite(dist_out_prev):
        _cur_cfg = FAR_CFG.copy()
        return "FAR"
    if dist_out_prev < THRESH_NEAR:
        _cur_cfg = NEAR_CFG.copy()
        return "NEAR"
    elif dist_out_prev < THRESH_MID:
        _cur_cfg = MID_CFG.copy()
        return "MID"
    else:
        _cur_cfg = FAR_CFG.copy()
        return "FAR"
...
def d4pg_policy_action(agent: D4PGAgent, state_vec: np.ndarray,
                       dist_out_prev: float, do_learn: bool,
                       global_step: int):
    """
    Lấy action từ actor của D4PG, thêm Gaussian noise nếu đang TRAIN.
    """
    state_np = np.asarray(state_vec, dtype=np.float32)
    state_tensor = torch.from_numpy(state_np).unsqueeze(0)

    with torch.no_grad():
        act = agent.actor(state_tensor).cpu().numpy()[0]

    if do_learn:
        # noise scheduling đơn giản theo khoảng cách
        base_max = 0.20  # biên độ noise max
        if np.isfinite(dist_out_prev):
            scale = np.clip(dist_out_prev / THRESH_MID, 0.0, 1.0)
        else:
            scale = 1.0
        sigma = base_max * (0.20 + 0.80 * scale)
        noise = np.random.normal(scale=sigma, size=act.shape)
...
def _run_one_episode(
    node, agent: D4PGAgent, do_learn: bool, episode_idx: int, horizon: int,
    ...
    ep_ret = 0.0
    if VERBOSE_RL:
        tag = "TRAIN-D4PG" if do_learn else "DEMO-D4PG"
    ...
    for t in range(horizon):
        ...
        # ===== action từ policy D4PG =====
        a = d4pg_policy_action(agent, s, dist_out_prev, do_learn, global_step_local)
        q_target = scale_action_unit_to_joint(a)
        ...
        if do_learn and danger_inside:
            ...
            agent.step_training(s, a, reward, s, done, BATCH_SIZE, PER_MEMORY_FLAG)
            # log cuối
        ...
        # ===== học / log =====
        if do_learn:
            agent.step_training(s, a, reward, s_next, done, BATCH_SIZE, PER_MEMORY_FLAG)
        ...
        if VERBOSE_RL and (t % LOG_EVERY_STEP == 0):
            ...
            print(f"[RL-{tag}-D4PG] mode={mode} | d_surf(prev)={dist_out_prev:.3f}@{prev_tag} "
...
def main(args=None):
    ...
    agent = D4PGAgent(
        ...
        n_steps=1,
        learn_every=1,  # mỗi step học một lần (cho nhanh)
    )
    ...
    try:
        if MODE.lower() == "train":
            ...
            for ep in range(1, NUM_EPISODES_TRAIN + 1):
                ep_ret = _run_one_episode(node, agent, do_learn=True,
                ...
                print(f"[D4PG-TRAIN] episode: {ep:4d} | return: {ep_ret:8.2f} | avg10: {avg10:8.2f}")
                # Lưu checkpoint sau mỗi EP với suffix _epXXXX
                try:
                    agent.save(CKPT_PREFIX, episode=ep)
                except Exception as e:
                    print(f"[CKPT-D4PG] save() failed: {e}")
            logger.close()
            print("[TRAIN-D4PG] Done. Exit.")

        else:
            # DEMO: nạp policy đã train, không học
            ckpt_list = sorted(glob.glob(f"{CKPT_PREFIX}_ep*.pt"))
            if len(ckpt_list) == 0:
                print(f"[CKPT-D4PG] No checkpoint found matching: {CKPT_PREFIX}_ep*.pt")
                return

            # chọn ngẫu nhiên NUM_EPISODES_DEMO checkpoint (không trùng nếu đủ)
            if len(ckpt_list) >= NUM_EPISODES_DEMO:
                demo_ckpts = random.sample(ckpt_list, NUM_EPISODES_DEMO)
            else:
                demo_ckpts = [random.choice(ckpt_list) for _ in range(NUM_EPISODES_DEMO)]

            logger = EpisodeCSVLogger(LOG_DIR, run_tag=RUN_TAG_DEMO)
            for ep_idx, ckpt_path in enumerate(demo_ckpts, start=1):
                try:
                    agent.load_from_file(ckpt_path)
                    print(f"[CKPT-D4PG] loaded checkpoint for DEMO EP {ep_idx}: {ckpt_path}")
                except Exception as e:
                    print(f"[CKPT-D4PG] load() failed for {ckpt_path}: {e} — skip EP")
                    continue

                _ = _run_one_episode(node, agent, do_learn=False,
                                     episode_idx=ep_idx, horizon=EPISODE_HORIZON,
